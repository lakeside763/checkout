        ...
        <SimpleGrid columns={[1, null, 2, null, 4]} spacing={10}>
          {
            products.map(({ id, image, price, title, averageRating, totalReviews }) => (
              <Box key={id} w={{ base: '100%'}} borderWidth="1px" borderRadius="md">
                <Link href={`/${id}`}>
                  <a>
                    <Image src={`${image}`} width='400px' height='400px' alt='Product image'/>
                    <Box pt='2' px='6' pb='8'>
                      <Heading fontSize='large' fontWeight='10px'>{title}</Heading>
                      <Heading fontSize='large' mt='4'>${price}</Heading>
                      <Flex fontSize='12px' mt='1'>
                        <Text>Sold by </Text>
                        <Text fontWeight='bold' color='#033574'>&nbsp;Checkout</Text>
                      </Flex>
                      <Flex mt='3'>
                        {Array(5)
                        .fill('')
                      ...
                      <Button mt='5' variant='primary'>Add to Cart</Button>
                    </Box>
                  </a>
                </Link>
